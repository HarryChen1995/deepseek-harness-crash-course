...
export function apply(ctx: Context) {
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      top: { type: 'integer', description: 'How many long words to return (1-10, default 3)' },
    ...
    output: {
      ...
      render: (_args, value) => [{
        ...
        text: [
          ...
          `longest: ${value.longest.join(', ') || '(none)'}`,
        ].join('\n'),
      }],
    ...
    async execute(args, exec) {
      ...
      const stopwords = new Set(
        (args.ignore?.stopwords ?? []).map((w) => args.ignore?.caseSensitive ? w : w.toLowerCase()),
      )

      const words = args.text
        .split(/\s+/)
        .filter(Boolean)
        .filter((w) => !stopwords.has(args.ignore?.caseSensitive ? w : w.toLowerCase()))
